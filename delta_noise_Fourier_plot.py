...
from heuristics import cpqr_select, dopt_greedy_select, dopt_select, random_select
...
delta_noise_dopt_lst = []
delta_noise_ub_dopt_lst = []
...
for num_modes in mode_range:
    ...
    _, delta_noise_greedyDopt, _, delta_noise_ub_greedyDopt = risk_prem(U_r=U_r, 
                                                                        ...
                                                                        sigma_noise=sigma_noise)
    _, delta_noise_dopt, _, delta_noise_ub_dopt = risk_prem(U_r=U_r, 
    ...
    delta_noise_dopt_lst.append(delta_noise_dopt)
    delta_noise_ub_dopt_lst.append(delta_noise_ub_dopt)
...
plt.text(num_sensors, 400, f'sensors = {num_sensors}', fontsize=14, 
        horizontalalignment='center', color='black') # 21.2*(10**4)  #186

plt.semilogy(mode_range, delta_noise_random_lst, marker='o', markersize=3, 
...
alpha = .7
plt.semilogy(mode_range, delta_noise_cpqr_lst, alpha=alpha, marker='o', markersize=3, 
             label=r'$\delta_{\textrm{noise}}$ (CPQR)', color='darkgoldenrod')
plt.semilogy(mode_range, delta_noise_ub_cpqr_lst, alpha=alpha,
             label=r'$\delta_{\textrm{noise}}$ u.b. (CPQR)', linestyle='--', color='darkgoldenrod')
plt.semilogy(mode_range, delta_noise_greedyDopt_lst, alpha=alpha, marker='o', markersize=3,
...
plt.semilogy(mode_range, delta_noise_dopt_lst, alpha=alpha, marker='o', markersize=3,
             label=r'$\delta_{\textrm{noise}}$ (D-opt.)', color='blue') 
plt.semilogy(mode_range, delta_noise_ub_dopt_lst, alpha=alpha,
             label=r'$\delta_{\textrm{noise}}$ u.b. (D-opt.)', linestyle='--', color='blue') 
