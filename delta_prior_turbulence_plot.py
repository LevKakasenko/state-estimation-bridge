...
delta_prior_qmap_lst = []
delta_prior_ub_qmap_lst = []
...
for num_modes in mode_range:
    ...
    delta_prior_greedyDopt, _, delta_prior_ub_greedyDopt, _ = risk_prem(U_r=U_r, 
                                                                        ...
                                                                        sigma_noise=sigma_noise)
    delta_prior_qmap, _, delta_prior_ub_qmap, _ = risk_prem(U_r=U_r, 
    ...
    delta_prior_qmap_lst.append(delta_prior_qmap)
    delta_prior_ub_qmap_lst.append(delta_prior_ub_qmap)
...
plt.text(num_sensors, 45500, f'sensors = {num_sensors}', fontsize=14, 
        horizontalalignment='center', color='black') # 21.2*(10**4)  #186

plt.plot(mode_range, delta_prior_random_lst, 
...
alpha = .75
plt.plot(mode_range, delta_prior_cpqr_lst, alpha=1,
         label=r'$\delta_{\textrm{prior}}$ (CPQR)', color='darkgoldenrod') 
plt.plot(mode_range, delta_prior_ub_cpqr_lst, alpha=1,
         label=r'$\delta_{\textrm{prior}}$ u.b. (CPQR)', linestyle='--', color='darkgoldenrod')
plt.plot(mode_range, delta_prior_greedyDopt_lst, alpha=alpha,
...
plt.plot(mode_range, delta_prior_qmap_lst, alpha=alpha,
         label=r'$\delta_{\textrm{prior}}$ (Q-MAP)', color='blue') 
plt.plot(mode_range, delta_prior_ub_qmap_lst, alpha=alpha,
         label=r'$\delta_{\textrm{prior}}$ u.b. (Q-MAP)', linestyle='--', color='blue')
